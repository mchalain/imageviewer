...
define cmd_check_lib
	...
	$(Q)PKG_CONFIG_PATH=$(sysroot)/usr/lib/pkg-config:$(builddir) $(PKGCONFIG) --exists --print-errors $(CHECKOPTIONS) $(CHECKLIB);
	...
	$(Q)$(TARGETLD) -o $(TMPDIR)/$(TESTFILE) $(<:%.c=%.o) $(INTERN_LDFLAGS) $(CHECKLDFLAGS) > /dev/null 2>&1
endef
...
define cmd_generate_config_h
	...
	$(foreach config,$2,$(if $(findstring n,$($(config))),,$(file >> $@,#define $(config) $($(config))))) \
	$(file >> $@,) \
...
define cmd__saveconfig
 $(file > $@,)
 $(foreach config,$(2),$(file >> $@,$(config)=$($(config))))
endef
